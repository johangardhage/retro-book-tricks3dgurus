...
void DEMO_Render(double deltatime)
{
	...
	// draw the sky
	Draw_Rectangle(0, 0, screen_width - 1, screen_height / 2, RGB(0, 140, 192), framebuffer, framebuffer_pitch);

	// draw the ground
	Draw_Rectangle(0, screen_height / 2, screen_width - 1, screen_height - 1, RGB(115, 42, 16), framebuffer, framebuffer_pitch);
	...
	// draw instructions
	Draw_Text_GDI("Press ESC to exit. Press Arrow Keys to Move. Space for TURBO.", 0, 0, RGB(0, 255, 0), framebuffer, framebuffer_pitch);

	// reset number of polys rendered
	debug_polys_rendered_per_frame = 0;

	// render the object
	Draw_RENDERLIST4DV1_Solid(&rend_list, framebuffer, framebuffer_pitch);
...
void DEMO_Initialize(void)
{
	...
	Load_OBJECT4DV1_PLG(&obj_tank, "assets/tank3_8b.plg", &vscale, &vpos, &vrot);

	// load player object for 3rd person view
	VECTOR4D_INITXYZ(&vscale, 0.75, 0.75, 0.75);
	Load_OBJECT4DV1_PLG(&obj_player, "assets/tank2_8b.plg", &vscale, &vpos, &vrot);

	// load the master tower object
	VECTOR4D_INITXYZ(&vscale, 1.0, 2.0, 1.0);
	Load_OBJECT4DV1_PLG(&obj_tower, "assets/tower1_8b.plg", &vscale, &vpos, &vrot);

	// load the master ground marker
	VECTOR4D_INITXYZ(&vscale, 3.0, 3.0, 3.0);
	Load_OBJECT4DV1_PLG(&obj_marker, "assets/marker1_8b.plg", &vscale, &vpos, &vrot);
