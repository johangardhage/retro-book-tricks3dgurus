...
//	Draw_Bitmap16(&background_bmp, framebuffer, framebuffer_pitch, 0);
	...
	// draw instructions
	Draw_Text_GDI16("Press ESC to exit. Press <H> for Help.", 0, 0, RGB16Bit(255, 255, 255), framebuffer, framebuffer_pitch);
	...
	if (help_mode == 1) {
		...
		Draw_Text_GDI16("<Z>..............Toggle Z-sorting.", 0, text_y += 12, RGB16Bit(255, 255, 255), framebuffer, framebuffer_pitch);
		Draw_Text_GDI16("<1>,<2>..........Previous/Next Animation.", 0, text_y += 12, RGB16Bit(255, 255, 255), framebuffer, framebuffer_pitch);
		Draw_Text_GDI16("<3>,<4>..........Play Animation Single Shot/Looped.", 0, text_y += 12, RGB16Bit(255, 255, 255), framebuffer, framebuffer_pitch);
	...
	sprintf(work_string, "Lighting [%s]: Ambient=%d, Infinite=%d, Point=%d, BckFceRM [%s], Zsort[%s]",
		...
		((zsort_mode == 1) ? "ON" : "OFF"));
	Draw_Text_GDI16(work_string, 0 + 1, screen_height - 34 + 1, RGB16Bit(0, 0, 0), framebuffer, framebuffer_pitch);
	Draw_Text_GDI16(work_string, 0, screen_height - 34, RGB16Bit(255, 255, 255), framebuffer, framebuffer_pitch);

	sprintf(work_string, "Polys Rendered: %d, Polys lit: %d Anim[%d]=%s Frm=%f", debug_polys_rendered_per_frame, debug_polys_lit_per_frame, obj_md2.anim_state, md2_anim_strings[obj_md2.anim_state], obj_md2.curr_frame);
	Draw_Text_GDI16(work_string, 0 + 1, screen_height - 34 - 2 * 16 + 1, RGB16Bit(0, 0, 0), framebuffer, framebuffer_pitch);
	Draw_Text_GDI16(work_string, 0, screen_height - 34 - 2 * 16, RGB16Bit(255, 255, 255), framebuffer, framebuffer_pitch);

	sprintf(work_string, "CAM [%5.2f, %5.2f, %5.2f], CELL [%d, %d]", cam.pos.x, cam.pos.y, cam.pos.z, cell_x, cell_y);
	Draw_Text_GDI16(work_string, 0 + 1, screen_height - 34 - 3 * 16 + 1, RGB16Bit(0, 0, 0), framebuffer, framebuffer_pitch);
	Draw_Text_GDI16(work_string, 0, screen_height - 34 - 3 * 16, RGB16Bit(255, 255, 255), framebuffer, framebuffer_pitch);
...
void DEMO_Initialize(void)
{
	...
	Load_Bitmap_File(&bitmap16bit, "assets/sunset800_600_03.bmp");
	...
	Load_Object_MD2(&obj_md2,                            // the loaded md2 file placed in container
		"assets/tris.md2", //  "D:/Games/quakeII/baseq2/players/male/tris.md2",    // the filename of the .MD2 model
		&vs,
		&vp,
		NULL,
		"assets/blade_black.bmp", //"D:/Games/quakeII/baseq2/players/male/claymore.bmp",   // the texture filename for the model
