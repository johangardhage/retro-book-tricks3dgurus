// T3DLIB13.H - header file for T3DLIB13.CPP
...
unsigned long int lpFrequency;
unsigned long int lpPerformanceCount;
unsigned long int ticks_per_ms;
...
int Load_Object_MD2(MD2_CONTAINER_PTR obj_md2, // the loaded md2 file placed in container
	const char *modelfile,    // the filename of the .MD2 model
	VECTOR4D_PTR scale, // initial scaling factors
	VECTOR4D_PTR pos,   // initial position
	VECTOR4D_PTR rot,   // initial rotations (not implemented)
	const char *texturefile,  // the texture filename for the model
...
void Wait_Fast_Timer(int delay)
{
	...
	unsigned long int lpCurrPerformanceCount;

	while (1) {
		// get current time
		lpCurrPerformanceCount = SDL_GetPerformanceCounter();
